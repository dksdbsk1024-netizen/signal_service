...
# 분봉 1건은 240봉 채우는 데 8회 호출이 든다. 봉은 1분에 한 번만 바뀐다.
# 90초인 이유: 화면 폴링이 60초 주기다. TTL 이 폴링보다 짧으면(예전 30초) 캐시가 폴링을
# 단 한 번도 흡수하지 못하고 매 틱이 8회 GET 을 그대로 KIS 로 보낸다. 폴링 주기보다 길게
# 잡아야 캐시가 산다. 대신 새 봉을 최대 90초 늦게 볼 수 있는데, 60초 주기로 그리는
# 차트에서 한 틱 지연은 잃는 게 없다.
